...
public class DatabaseTableModel extends TableModel
{
		...
		else
		{
			...
			else
			{
				...
				ImmutableList<ImmutableList<ColumnVariable>> results = Database.select(new Selection(modelTable), 
						getTable(), ImmutableList.withValue(new Join(references.head(), Join.JoinType.INNER)),
	...
	private Option<Condition> getCondition()
	{
		...
		else if (this.columnComboKeys.length > 0)
		{
			...
			return CombinedCondition.combineConditions(CombinedCondition.CombinationOperator.AND, conditions);
		}
		else
			return Option.none();
	}
				...
				throws utopia.java.flow.generics.VariableParser.VariableGenerationFailedException
		{
			try
			{
				...
				if (getTable().containsColumnForVariable(variableName))
				{
					...
					utopia.java.flow.generics.Model.NoSuchAttributeException e)
			{
